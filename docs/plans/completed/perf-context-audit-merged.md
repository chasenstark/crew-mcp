...
| 14 | **Cap `RunStateV1.prompts[].prompt` storage.** Verbatim user prompts retained on disk forever; truncate at e.g. 16KB with marker. Refs: `run-state.ts:165–195` (`create`), `:286–298` (`appendPrompt`). Wire payloads now elide prompt text but disk growth + parse cost remain. **Resolution: 16K-char default cap with byte-count marker; `CREW_PROMPT_STORAGE_CAP_CHARS=0` disables.** | S | L | 🟢 | ✅ `9268804` | Claude §3.6 |
...
| # | Task | Eff | Imp | Status | Done | Source |
|---|------|-----|-----|--------|------|--------|
| 17 | **Fix doc drift: "6-tool surface" → 8** *(was 7 — `list_runs` joined the surface)*. `orchestrator/index.ts:8` comment now off by 2; `serve.ts:6–16` lists eight (`list_agents`, `list_runs`, `run_agent`, `continue_run`, `merge_run`, `discard_run`, `get_run_status`, `cancel_run`). | S | L (doc) | 🟠 | ✅ `fd2893e` | Codex X2 |
| 18 | **Skip post-run `git status` for adapters that report file-changes reliably.** Adapter capability flag; keep fallback for unreliable sources. Refs: `adapters/types.ts:240–244` (`TaskResult.filesModified`), `run-agent.ts:368–380` (always merges). **Resolution: `filesModifiedReliable?: boolean` on AgentAdapter; only Codex marked true (structured `file_change` events). Claude/Gemini/openai-compat/generic stay on the git-status fallback until their parsers track edits reliably.** | S | L-M | 🟢 | ✅ `1ff5fc8` | Codex S9 |
| 19 | **Compact MCP `content` text — stop pretty-printing structured payloads.** `jsonContent` ships pretty JSON in `content[0].text` AND the same object in `structuredContent`. Hosts that retain text in transcript pay 2× context. **Impact bumped M-H → H:** envelope grew (`tail_url`, `tail_command_url`, `worktree_path` all duplicated). Refs: `serve.ts:1456–1462` (`jsonContent`), `:262, :278` (`list_agents` / `list_runs`). **Resolution:** `jsonContent` now ships compact JSON; `merge_run` / `discard_run` / `cancel_run` / `get_run_status` ship per-tool short markdown in `content[0].text` instead of pretty-printed envelope JSON; `list_agents` / `list_runs` use `jsonContent` (compact). `structuredContent` shapes unchanged. Representative byte savings on terminal `get_run_status`: 744 → 333 chars in `content[0].text` (-55%). | M | H | 🟢 | ✅ `c1ecafc` | Codex C2 |
...
| N3 | **`crew-wait` per-watched-run polling cost.** New Node process spawned for every Claude Code dispatch; polls `state.json` every 1s (`cli/wait.ts:8, :77–98, :124–132`). Accepted trade for chat-availability. **Parallel dispatch is now an actively-encouraged pattern** (e.g., dispatching multiple reviewers / multiple model implementations side-by-side), so this scales linearly with how the workflow is actually being used. **Resolution: kept one process per run (preserves Claude Code synthetic-turn contract) but replaced 1s busy-poll with directory `fs.watch`. Atomic tmp+rename writes handled; initial-race tolerated; polling fallback at 2s base / 5s backoff for unsupported platforms (`CREW_WAIT_POLL_INTERVAL_MS` configures). Cold-start cost (~50–200ms per dispatch) is intrinsic and unchanged.** | M | L-M | ✅ `1df9b8f` (with N7) | ✅ Both |
...
**Third batch shipped (also 2026-05-10).** Five more items: `1df9b8f` (N3 + N7 — `crew-wait` now uses `fs.watch` instead of busy-poll), `1ff5fc8` (#18 — `filesModifiedReliable` adapter capability flag; only Codex marked true), `fd2893e` (#17 — fixed "6-tool surface" doc drift), `9268804` (#14 — 16K-char prompt-storage cap with marker). Full test suite: 800 passed, 3 skipped post-merge.

**Fourth batch shipped (also 2026-05-10).** Two Tier 2 items: `55efdba` (#8 — `readFilteredTailFromEnd` primitive in `RunStateStore`; terminal `get_run_status` now scans `events.log` once instead of twice; per-run mtime cursor cache remains parked), `eecaa5f` (#9 — captain skill body condensed 508→437 lines via overlap and rationale cuts; pre-dispatch ask gate strengthened; cut prose preserved in `docs/captain-skill-body-cuts-2026-05-10.md`). Full test suite: 808 passed, 3 skipped post-merge (8 new tests for #8's tail-from-end primitive).

**Fifth batch shipped (also 2026-05-10).** One Tier 4 item: `c1ecafc` (#19 — compact MCP `content` text; `jsonContent` drops pretty-print indentation; `merge_run` / `discard_run` / `cancel_run` / `get_run_status` ship per-tool short markdown instead of re-rendering structured envelope; `list_agents` / `list_runs` use `jsonContent`. `structuredContent` shapes unchanged. Representative terminal `get_run_status`: 744→333 chars in `content[0].text` (-55%)). Full test suite: 810 passed, 3 skipped post-merge (2 new tests). Cherry-picked into main rather than merged via `merge_run` to avoid bringing in unrelated host-state-mirror file (`inbox-send-message.md`); run discarded.
