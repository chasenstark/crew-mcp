...
|------|---------------|-------------|
| `run_agent` | dispatched | **Primary work primitive.** Delegate a bounded task to a named subagent. Allocates `~/.crew/runs/<runId>/worktree/`; the dispatcher's terminal-event listener cleans up. Write the agent's prompt inline — do NOT route through `plan_tasks` for single-task work. Input: `{agent_id, prompt, working_directory?, model?, effort?, read_only?}`. `effort` (`low|medium|high|xhigh|max`) overrides the per-machine default in `~/.crew/agents.json`. `read_only: true` skips worktree allocation entirely — the agent runs against `working_directory` (defaulting to the host repo root) without FS isolation; `merge_run` refuses, `discard_run` is metadata-only, post-run dirty-tree probe surfaces a `warnings` field if the agent edited despite the contract. Streams chunks via `notifications/progress` when client supplies a `progressToken`; blocks up to 60s before returning `status: "running"` for the captain to poll. (Was 5min pre-2026-05; reduced because hosts that don't surface progress notifications turned long blocks into a UX dead-zone.) |
