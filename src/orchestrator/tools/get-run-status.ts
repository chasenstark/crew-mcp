...
export const GET_RUN_STATUS_DESCRIPTION =
  `Poll a run by run_id. Always poll after run_agent / continue_run — those return status:"running" immediately. **Always pass wait_for_change_ms: 30000** plus since_event_line (the prior response's next_event_line). The server blocks until new events arrive or the run reaches terminal state (and returns immediately if already terminal), so each poll wakes the captain on real progress. Snapshot polls (no wait_for_change_ms) return instantly and create tight polling loops — avoid. While running, response is lean — status + cursor only. On terminal, response adds summary (last turn's output), filesChanged, prompts, and events_tail (default ${DEFAULT_MAX_EVENTS_TAIL}; configurable via max_events_tail up to ${MAX_EVENTS_TAIL_CAP}).`;
